...
const EPISODE_STEP_MS      = 5 * 60 * 1000; // 5-minute sample resolution
const EPISODE_MAX_LOOKBACK = 48 * 3600 * 1000;
const EPISODE_MAX_AHEAD    = 24 * 3600 * 1000;

let episodeCurve     = null; // precomputed; rebuilt every minute or on data change
...
function buildEpisodeCurve() {
    ...
    if (currentLevel >= THRESH) {
        // Active episode — walk back to find last time below threshold
        ...
        // Project forward to find clearance time
        ...
        // No active episode — find last complete episode (went above then below threshold)
        let lastAboveMs = null;
        for (let t = nowMs - EPISODE_STEP_MS; t >= nowMs - EPISODE_MAX_LOOKBACK; t -= EPISODE_STEP_MS) {
            if (computeLevelAt(t, entries, halfLifeMs) >= THRESH) { lastAboveMs = t; break; }
        }
        if (!lastAboveMs) {
            // Nothing in 48h — show last 12h
            episodeStartMs = nowMs - 12 * 3600 * 1000;
            episodeEndMs   = nowMs;
        } else {
            // Find when that episode started and ended
            episodeStartMs = nowMs - EPISODE_MAX_LOOKBACK;
    ...
    // Add padding: 30 min either side
...
function updateEpisodeSubtitle() {
    ...
    if (currentLevel >= EPISODE_THRESHOLD_MG) {
        const ago      = fmtDuration(nowMs - episodeStartMs);
        const clearsIn = fmtDuration(episodeEndMs - nowMs);
        sub.textContent = `Episode started ${ago} ago · clears in ~${clearsIn}`;
    } else {
        const ended = fmtDuration(nowMs - episodeEndMs);
        sub.textContent = ended === '0m'
            ? 'No active episode'
            : `No active episode · last ended ${ended} ago`;
    }
}
...
function drawEpisodeFrame() {
    ...
    const tMin  = episodeStartMs;
    const tMax  = episodeEndMs;
    ...
    const tickStep = totalMs > 36 * 3600000 ? 6 * 3600000
                   : totalMs > 18 * 3600000 ? 3 * 3600000
                   : 2 * 3600000;
    ...
    // Snap first tick to a clean hour
    ...
    // Interpolate "now" position on the curve
    let nowMg = 0;
    ...
    // Animated "now" dot (pulsing)
    const pulse = 1 + 0.2 * Math.sin(Date.now() / 600);
    ctx.beginPath();
    ctx.arc(nowX, nowY, 7 * pulse, 0, Math.PI * 2);
    ctx.fillStyle = dark ? 'rgba(102,126,234,0.2)' : 'rgba(102,126,234,0.15)';
    ctx.fill();

    ctx.beginPath();
    ctx.arc(nowX, nowY, 4.5, 0, Math.PI * 2);
    ctx.fillStyle = '#667eea';
    ctx.fill();
    ctx.beginPath();
    ctx.arc(nowX, nowY, 2.5, 0, Math.PI * 2);
    ctx.fillStyle = isDarkMode() ? '#1a1b2e' : 'white';
    ctx.fill();
